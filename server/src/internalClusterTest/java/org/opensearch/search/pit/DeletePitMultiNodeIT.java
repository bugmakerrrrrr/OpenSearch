        ...
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new Thread(() -> {
                ...
                try {
                     ...
                     */
                    if (deleted.get() == true) {
                        assertTrue(e.toString(), e.getCause() instanceof SearchPhaseExecutionException);
                        assertTrue(e.toString(), e.getCause().getCause() instanceof SearchContextMissingException);
                        return;
                    }
                    throw new AssertionError(e);
                }
